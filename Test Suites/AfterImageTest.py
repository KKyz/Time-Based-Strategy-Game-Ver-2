import pygame #Calls in pygame library
import random #Calls in random integers
import gzip #?
import base64 #Calls in base64 library

TILESIZE = 64 #Defines size of every tile (the smaller it is, the smaller the actual screen estate)
GRID_W, GRID_H = (10, 7) #Describes how many grids there will be on screen
LOOKUP = None #?

class Shadow(pygame.sprite.Sprite): #Defines The Afterimage Special Effect
    def __init__(self, source):
        super().__init__()
        self._layer = 5 #Defines On which layer the afterimages will appear
        self.image = source.image.copy().convert_alpha() #Copies From Main Image
        self.image.fill((0, 0, 200, 100), special_flags=pygame.BLEND_ADD) #Redefines Colors On Characters
        self.rect = source.rect.copy() #Copies Self Image
        self.timeout = 200 #Makes Another Afterimage appear every 200 miliseconds

    def update(self, events, dt): #How to update on every frame
        self.timeout -= dt #Subtracts the creation intervals with the framerate
        if self.timeout <= 0: #If the result is less than 0...
            self.kill() #Cancel out yourself

class Actor(pygame.sprite.Sprite): #Definition of the main character
    def __init__(self, grid_pos):
        super().__init__()
        self._layer = 10 # Defines on which layer the main character will appear
        data, size = gzip.decompress(base64.b64decode(HYDRA)), (64, 64) #decrypts character image
        self.image = pygame.image.fromstring(data, size, "RGB") #defines character image
        self.image.set_colorkey((71, 108, 108)) # defines colors for character
        self.rect = self.image.get_rect()
        self.pos = pygame.Vector2() #Uses vector 2 to decide position
        self.update_pos(grid_pos)
        self.timeout = 1000 #moves every 1000 milliseconds
        self.shadow_timeout = 100 

    def update_pos(self, grid_pos): #Update Position
        self.target_pos = None #Norally, there is no targeted position
        self.target_grid_pos = None #Normally, there is no targeted grid position
        self.grid_pos = grid_pos # Grid Coordiantes
        self.rect.center = get_grid_rect(grid_pos).center #Center Yourself
        self.pos = pygame.Vector2(self.rect.center) # Self Position

    def move_random(self):
        d = random.choice([-1, 1])
        x, y = self.grid_pos
        if random.randint(0, 2):
            x += d
        else:
            y += d
        self.target_pos = pygame.Vector2(get_grid_rect((x, y)).center)
        self.target_grid_pos = (x, y)

    def update(self, events, dt): #Update Self
        self.timeout -= dt #Movement
        if self.timeout <= 0:
            self.timeout = 1000
            self.move_random()
        if self.target_grid_pos: #
            self.shadow_timeout -= dt
            if self.shadow_timeout <= 0:
                self.shadow_timeout = 100
                self.groups()[0].add(Shadow(self))
            self.pos = self.pos.lerp(self.target_pos, 0.1)
            if self.pos.distance_to(self.target_pos) < 1:
                self.update_pos(self.target_grid_pos)
        self.rect.center = self.pos

def get_grid_rect(pos):
    x, y = pos
    return LOOKUP[y][x]

def create_grid():
    surf = pygame.Surface((TILESIZE * GRID_W, TILESIZE * GRID_H))
    surf.set_colorkey((2, 2, 2))
    surf.fill((2, 2, 2))
    grid = []
    for y in range(GRID_H):
        line = []
        for x in range(GRID_W):
            r = pygame.Rect(x * TILESIZE, y * TILESIZE, TILESIZE, TILESIZE)
            line.append(r)
            pygame.draw.rect(surf, pygame.Color('grey'), r, 1)
        grid.append(line)

    return grid, surf

def main():
    screen = pygame.display.set_mode((TILESIZE * GRID_W, TILESIZE * GRID_H))
    dt, clock = 0, pygame.time.Clock()
    grid, grid_surf = create_grid()
    global LOOKUP 
    LOOKUP = grid
    sprites = pygame.sprite.LayeredUpdates(Actor((4, 3)))
    while True:
        events = pygame.event.get()
        for e in events:
            if e.type == pygame.QUIT:
                return
        sprites.update(events, dt)
        screen.fill((30, 30, 30))
        screen.blit(grid_surf, (0, 0))
        sprites.draw(screen)
        pygame.display.flip()
        dt = clock.tick(60)

HYDRA = ''

if __name__ == '__main__':
    main()